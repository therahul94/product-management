...
//validate first name
    ...
    if (!validator.validString(fname)) {
      return res
        .status(400)
        .send({ status: false, message: "fname should be string." });
    }
    if (!validator.validString(lname)) {
      return res
        .status(400)
        .send({ status: false, message: "lname should be string." });
    }
    ...
    if (shipping) {
      if (!shipping.street)
        return res
          .status(400)
          .send({ status: false, message: "shipping street is missing." });
      // if(!(.test(shipping.street))) return res.status(400).send({Status: false , message:"Please enter the valid shipping street address"})

      if (!shipping.city)
        return res
          .status(400)
          .send({ status: false, message: "shipping city is missing." });
      if (!/^[a-zA-Z]+$/.test(shipping.city))
        return res
          .status(400)
          .send({
            Status: false,
            message: "Please enter the valid shipping city address",
          });
      ...
      if (!/^[1-9][0-9]{5}$/.test(shipping.pincode))// ([1-9]{1}[0-9]{5}|[1-9]{1}[0-9]{3}\\s[0-9]{3})
        return res
          ...
          .send({
            Status: false,
            message: "Please enter the valid shipping pincode ",
          });
    } else
      return res
        .status(400)
        .send({ status: false, message: "Shipping field is not given" });

    if (billing) {
      if (!billing.street)
        return res
          .status(400)
          .send({ status: false, message: "billing street is missing." });
      // if(!(.test(shipping.street))) return res.status(400).send({Status: false , message:"Please enter the valid billing street address"})

      if (!billing.city)
        return res
          .status(400)
          .send({ status: false, message: "billing city is missing." });
      if (!/^[a-zA-Z]+$/.test(shipping.city))
        return res
          .status(400)
          .send({
            Status: false,
            message: "Please enter the valid billing city address",
          });

      if (!billing.pincode)
        return res
          .status(400)
          .send({ status: false, message: "billing pincode is missing." });
      if (!/^[1-9][0-9]{5}$/.test(shipping.pincode))
        return res
          .status(400)
          .send({
            Status: false,
            message: "Please enter the valid billing pincode ",
          });
    } else
      return res
        .status(400)
        .send({ status: false, message: "billing field is not given" });
    ...
    if (isSameEmail)
      return res
        ...
        .send({
          status: false,
          message: "Already registered Email, try different emailId",
        });
    ...
    if (isSamePhone)
      return res
        ...
        .send({
          status: false,
          message: "Already registered, try different Phone no.",
        });
    ...
    if (!(files && files.length)) {
      return res
        .status(400)
        .send({ status: false, message: "Image file is missing." });
    }
    ...
    return res
      .status(201)
      .send({ status: true, message: "User created successfully", data: data });
  } catch (error) {
    console.log(error);
  ...
  try {
    ...
    if (lname) {
      ...
      if (validator.validString(lname)) {
        return res
          .status(400)
          .send({ status: false, msg: "lname should be string" });
      }
    ...
    if (phone) {
      if (!validator.validMobileNum(phone)) {
        return res
          .status(400)
          .send({ status: false, msg: "Enter a valid phone number" });
      }
      if (!validator.validMobileNum(phone)) {
        return res
          .status(400)
          .send({ status: false, message: "Moblie num is invalid" });
      }
    ...
    if (address) {
      ...
      if (shipping) {
        ...
        if (typeof shipping != "object") {
          return res
            .status(400)
            .send({ status: false, message: "Shipping must be in object" });
        }
        ...
        if (pincode) {
          ...
          if (!/^\d{6}$/.test(pincode)) {
            return res
              ...
              .send({
                status: false,
                message: "plz enter valid  shipping pincode",
              });
          }
      ...
      if (billing) {
        if (typeof billing != "object") {
          return res
            .status(400)
            .send({ status: false, message: "Billing must be in object" });
        }
